...
is fully deterministic. Construct ids must be unique; duplicates are
refused rather than resolved to an arbitrary state slot.
...
A named list `list(states, dt, steps, k, damping, init, trajectory)`,
...
rounded to 6 decimals. All five knobs are echoed back, because the
