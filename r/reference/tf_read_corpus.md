...
Reads a corpus object (`{schema_version, id, records}`; see
`schema/corpus.schema.json`) into a named list. The format is chosen by
the file extension (`.json` -\> JSON, otherwise YAML). Mirrors the
Python `theoryforge.read_corpus(path)`.
