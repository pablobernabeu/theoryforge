...
- The bundled `panic-network` fixtures give the three constructs
  ...
  where construct scopes diverge rather than putting a zero in six of
  its seven regions.
...
- Documentation: the Get started vignette shows what
  ...
  in its default dry-run mode rather than withholding it.
...
- The DOT diagram views are redesigned for content and legibility. Every
  ...
  workflow and pipeline nodes carry the id with the relation or type
  rather than a bare word; the development roadmap stacks its items in a
  single column; and the theme landscape colours themes by status. Every
  view fits a documentation column. The intermediate representation
  stays byte-identical to the Python twin’s.
...
- The severity chart is re-laid out: bars start just past the longest
  ...
  accordingly; it stays byte-identical to the Python twin’s.
