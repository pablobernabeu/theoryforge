...
appends to the theory and records a provenance entry, and each returns
the theory, so the steps compose with the native pipe.
...
reads the same subgraph without leaving R, checking it for acyclicity
and returning the conditional independencies it entails, which the
Developing and testing article works through.
