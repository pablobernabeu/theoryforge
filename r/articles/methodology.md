...
and the R and Python implementations compute byte-identical results, so
a verdict is reproducible and can be audited against the specification.
...
are point or interval, and test severity passes when the mean computed
severity reaches 0.5.
...
- **Test severity** is the mean computed severity across the
  predictions, and passes at 0.5 or above.
