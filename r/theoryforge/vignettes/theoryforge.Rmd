...
tf_validate(theory, full = TRUE) # also checks referential integrity of ids and cross-references
```

`tf_validate()` returns `TRUE` invisibly on success and stops with a message
listing every problem otherwise. With `full = TRUE` it additionally checks
