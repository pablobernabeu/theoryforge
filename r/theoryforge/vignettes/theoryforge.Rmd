...
object. This vignette walks through the core loop: **building** a theory,
**checking** its rigor, and **diagramming** it. It is the R twin of the Python
package; both follow the same pinned contract (`API_SPEC.md`).
...
`tf_check()` runs the 12-item rigor checklist defined in the vendored
`rigor_checklist.yaml` and scored per `API_SPEC.md` section 4:
...
diagram types. The digraphs are Graphviz DOT; the causal DAG uses dagitty
syntax.
