# theoryforge: API and parity specification (v0.6.0, P0)
...
7. **Prediction severity** (file order): `prediction[i] severity must be a number between 0 and 1` when a prediction carries a `severity` that is not a single number within [0, 1] (booleans do not count as numbers). The schema already types `severity` this way; the `full` pass enforces it so a string severity cannot validate and then be refused at scoring time (§4 item 3).
...
3. **risk_severity.** `sevs = [p.severity for p in preds if p.severity is not None]`. A non-null severity that is not a number (strings, booleans, NaN) raises `check requires numeric prediction severities; non-numeric severity for prediction: <id>` in both languages, reporting the first offending prediction in file order; `validate(full)` reports the same file as invalid (§2 item 7). If empty: score 0.0, warn. Else `m = mean(sevs)`; score round(m,3); `pass` if `m ≥ thr.min_severity` else `warn`.
