...
- Causal testability passes if at least one proposition states a causal relation (causes, increases or decreases), so that the network exports as a directed graph. The export is not checked for acyclicity: a theory whose causal relations form a feedback loop, as the panic-network example does, is emitted as written and is not a valid dagitty DAG.
